...
function analyzeIncomes(targetMonthStr: string, targetLbpMonthStr: string) {
    ...
    console.log(`--- Revenus détectés en ${targetMonthStr} ---`);
    ...
    if (fs.existsSync(boursoPath)) {
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            if (date.startsWith(targetMonthStr) && amount > 0) {
    ...
    if (fs.existsSync(lbpPath)) {
        ...
        for (const line of lines) {
            ...
            if (date.includes(targetLbpMonthStr) && amount > 0) {
...
analyzeIncomes('2026-03', '/03/2026');
